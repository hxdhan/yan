...
if($get_users = $mysqli->query("SELECT invitee_userid,message FROM usrfriendinvite WHERE inviter_userid = $user_id")) {
...
foreach($all_users as $user) {
	if($get_user = $mysqli->query("SELECT ui.*,u.cellphone FROM userinfo ui, user u WHERE ui.user_id = u.user_id and  ui.user_id = {$user['invitee_userid']}")) {
